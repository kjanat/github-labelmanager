...
const { buildCommitUrl, getCommitSha, getInputs, loadOutputFile, run } = await import('~/run.ts');
	...
	beforeEach(() => {
		...
		mockOctokit.rest.issues.createComment.mockResolvedValue({
			data: {
				id: 999,
				body: '',
				html_url: 'https://github.com/test-owner/test-repo/issues/42#issuecomment-999',
			},
		});
	});
...
describe('run - push event without PR', () => {
	...
	it('logs publish info to console', async () => {
		...
		const hasPublishInfo = infoCalls.some(
			(msg: string) => msg.includes('Publish Information') || msg.includes('='.repeat(50)),
		);
