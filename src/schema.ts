...
export const labelNameBase = z.string()
  .min(1).max(50)
  .regex(/^(?!\s).*(?<!\s)$/)
  .meta({
    title: "Label Name",
...
export const hexColorBase = z.string()
...
export const labelDescriptionBase = z.string()
  .min(0).max(100)
  .meta({
...
export const labelName = labelNameBase.transform((val) =>
  LabelNameUtils.parse(val)
);

export const hexColor = hexColorBase.transform((val) =>
  LabelColorUtils.parse(val)
);

export const labelDescription = labelDescriptionBase.transform((val) =>
...
export const labelConfigSchema = z.strictObject({
