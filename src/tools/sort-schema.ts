...
export function sortSchema(obj: JsonValue): JsonValue {
	...
	entries.sort(([a], [b]) => {
		const pa = schemaPriority[a] ?? 1000;
		const pb = schemaPriority[b] ?? 1000;
...
async function main(): Promise<number> {
	...
	if (args._.length === 0) {
		...
		throw new Error('exactly one input file required');
	}
	...
	if (args['in-place'] && isStdin) {
		...
		throw new Error('--in-place cannot be used with stdin');
	}
